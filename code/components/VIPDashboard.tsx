...
import { type Demand, type Offer, type Reservation, DemandStatus, BookingStatus, ReservationStatus, type GeoJSONPoint } from "../types"
...
const VIPDashboard: React.FC<VIPDashboardProps> = ({ setView }) => {
  ...
  const getOfferStatusChip = (status: BookingStatus) => {
    const statusText = {
      [BookingStatus.Waiting]: t("status.waiting"),
      [BookingStatus.Negotiating]: t("status.negotiating"),
      [BookingStatus.Matched]: t("status.matched"),
    }[status]

    const statusColor = {
      [BookingStatus.Waiting]: "bg-amber-100 text-amber-800",
      [BookingStatus.Negotiating]: "bg-blue-100 text-blue-800",
      [BookingStatus.Matched]: "bg-emerald-100 text-emerald-800",
    }[status]
          ...
          const popupContent = `
            <div style="max-width: 280px;">
              ...
              <div style="margin-top: 8px; padding: 8px 0; border-top: 2px solid #0284c7;">
                ...
                <p style="font-size: 12px; margin-bottom: 4px;"><strong>${t('common.rate')}:</strong> ${offer.priceRate} MAD/jour</p>
